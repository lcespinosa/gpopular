...
export class TypeListComponent implements OnInit {
  ...
  listOfColumns: SearchableColumnItem[] = [
    ...
    },
    {
      name: 'Código',
      sortOrder: null,
      sortFn: (a: Type, b: Type) => a.code.localeCompare(b.code),
      sortDirections: ['ascend', 'descend'],
      searchable: false,
      searchVisible: false,
      searchValue: '',
  ...
  constructor(private typesApi: TypesService,
  ...
  updateList(): void {
    ...
    this.typesApi.getTypes()
      .subscribe((response: any) => {
        this.data = response.types;
        this.filteredData = response.types;
        this.loading = false;
      }, error => {
        this.loading = false;
      });
  ...
  editType(element: any): void {
    this.drawerTitle = 'Modificar tipos de caso';
    this.loading = false;
  ...
  storeType(): void {
    this.loading = true;
    this.typesApi.addType(this.validateForm.value)
      .subscribe((response) => {
        this.loading = false;
        this.updateList();
      });
  ...
  updateType(): void {
    this.loading = true;
    this.typesApi.updateType(this.validateForm.get('id').value, this.validateForm.value)
      .subscribe((response) => {
        this.loading = false;
        this.updateList();
      });
  ...
  onCurrentPageChanged(listOfCurrentPageData: Type[]): void {
  ...
  search(): void {
    ...
    this.filteredData = this.data.filter((item: Type) => item[column].indexOf(search) !== -1);
  }
